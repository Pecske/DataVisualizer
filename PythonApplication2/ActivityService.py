...
class ActivityService:
    ...
    def convert_dataframe_to_activities(self,df : DataFrame) -> dict[str,Activity] :
        rotated_data_frame : dict[str,int] = df.set_index(df.columns[0]).T.to_dict("list")
    ...
    def show_plot(self,table_data : TableData) -> None:
        plt.figure(table_data.name)
        plt.title(table_data.name)
        plt.tight_layout()
    ...
    def show_activity_plot_by_gender(self,activity_name : str, show_linear : bool) -> None:
        if(activity_name in self.activities):
            current_activity : Activity = self.activities.get(activity_name)
            gender_times : dict[str,dict[str,int]] = current_activity.get_group_by_gender()
